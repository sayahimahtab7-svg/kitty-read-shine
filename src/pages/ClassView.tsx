...
const ClassView = () => {
  ...
  const { classes, journal } = useAppStore();
  const cls = classes.find((c) => c.id === classId);
  const [activeTab, setActiveTab] = useState('journal');

  const notifCount = journal.filter((j) => !j.comment).length;

  if (!cls) return <div className="p-10 text-center text-muted-foreground">Class not found</div>;
      ...
      <div className="max-w-5xl mx-auto p-4">
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="journal" className="mt-4">
            <JournalTab classId={cls.id} />
          </TabsContent>
          <TabsContent value="activities" className="mt-4">
            <ActivitiesTab classId={cls.id} />
          </TabsContent>
          <TabsContent value="markbook" className="mt-4">
            <MarkbookTab classId={cls.id} />
          </TabsContent>
          <TabsContent value="notifications" className="mt-4">
            <NotificationsTab classId={cls.id} />
          </TabsContent>
        </Tabs>
        ...
        {activeTab === 'activities' && (
          <motion.button
            ...
            className="fixed bottom-8 right-8 gradient-primary text-primary-foreground w-16 h-16 rounded-full flex items-center justify-center shadow-button text-3xl font-bold"
          >
